...
/**
 * 合成材料道具前綴：grades「ABCDE」→ 改造圖A、改造圖B…改造圖E
 * （不是「改造圖 × 檔次 ABCDE」）
 */
export const REMODEL_BLUEPRINT_PREFIX = '改造圖';

/** @deprecated 請用 remodelBlueprintItems(grades) */
export const REMODEL_BLUEPRINT_ITEM = REMODEL_BLUEPRINT_PREFIX;

/** grades 字串如 "ABC" / "ABCDE" → ["改造圖A","改造圖B","改造圖C"] */
export function remodelBlueprintItems(grades: string): string[] {
