...
`compile` 依「寵物材料 → 寵物產物」有向邊算強連通分量；**成員數 ≥ 2** 才成群組（例如熊霸／酒仙／羅剎）。

`query.buildFusionTreeFromGraph`：

1. 外層：產物在群組、材料寵在群組外的 **取得** reaction → 仍用一般樹展開（stack 預填群組成員，避免嵌回循環）
2. 內層：一個 `cycleModule` 節點，群組內 reaction **各出現一次**
3. 多產物重抽永遠完整顯示全部產物與機率（不因從酒仙進入只顯示 2%）
4. 同一 `viewKey`（group id）：熊霸／酒仙／羅剎頁模組內容相同，只改 `focusedPet` 高亮
5. 材料若指向某循環群組成員 → 標「→ 某某系轉換」葉子，不遞迴複製

資料層可選 `FusionReaction.kind`（`acquire` | `reroll` | `convert`）；未填時由形狀推斷。
